    ...
    def __init__(
        ...
        for i in range(self.H):
            for j in range(self.W):
                ...
                # 则认为该空间位置无效
                # -------------------------------------------------
                mask = ~np.any(np.isnan(x), axis=0, keepdims=True)
        ...
        self.mask = np.asarray(mask_list, np.float32)
        ...
        if normalize:
            ...
            # 只统计有效区域
            valid_X = np.where(expanded_mask == 1, self.X, np.nan)

            self.X_mean = np.nanmean(
                valid_X,
                axis=(0, 2, 3),
                keepdims=True
            )

            self.X_std = np.nanstd(
                valid_X,
                axis=(0, 2, 3),
                keepdims=True
            ) + 1e-6
            ...
            # 无效区域重新置0
            self.X = np.where(expanded_mask == 1, self.X, 0.0)
