...
# 模块一：单模型全面精度分析 (基于测试集)
# ============================================================
def analyze_single_model_results():
    """基于 MLP 测试集 CSV 文件，绘制 4 张基础精度评估图"""
    csv_path = "./checkpoints/mlp/test_results.csv"
    output_dir = "./result/mlp"
    
    print(f"\n[模块一] 正在处理 MLP 单模型全面精度分析...")
    Path(output_dir).mkdir(parents=True, exist_ok=True)
    if not Path(csv_path).exists():
        print(f"    [跳过] 找不到 {csv_path}。请先运行 train_mlp.py 训练！")
        return
    ...
    plt.title(f"MLP Prediction vs Ground Truth")
    ...
    plt.ylabel("Density")
    plt.title(f"MLP Error Distribution (Bias={bias:.2f} m)")
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.xlabel("Depth (m)")
    plt.ylabel("Error (m)")
    plt.title("MLP Error Statistics vs. Depth")
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    plt.title(f'MLP Spatial Residual Map\nRMSE: {rmse:.2f} m')
...
def run_full_inference():
    """利用 InferenceDataset 进行纯 MLP 全域水深推断"""
    print(f"\n[模块二] 执行全域网格点对点推理 (MLP)...")
    ...
    OUTPUT_DIR = Path("./predictions/mlp")
    ...
    print("    -> 开始网络前向传播...")
    with torch.no_grad():
        x_tensor = dataset.X_valid_tensor
        batch_size = 16384  # MLP 极轻量，批次可以开到巨大
        predictions = []
    ...
    nc_out = OUTPUT_DIR / "predicted_bathymetry_mlp.nc"
    ds.to_netcdf(nc_out)
    ...
    plt.title(f'Predicted Bathymetry (MLP)')
