...
import json
from data.dataset_cnn import BathymetryInferenceDataset  # 改成适应CNN的数据集
...
def run_inference(model_path, output_dir="./predictions"):
    ...
    LON_RANGE = (105, 125)
    LAT_RANGE = (0, 30)
    NORM_PARAMS = "./checkpoints/cnn/normalization_params.json"  # 使用CNN的归一化参数
    ...
    dataset = BathymetryInferenceDataset(
        ...
        cache_dir="./cache/inference",  # 添加缓存目录
        lon_range=LON_RANGE,
        lat_range=LAT_RANGE,
        patch_size=5  # 与训练时保持一致
    )
    
    # 调试信息：检查数据集
    print(f"数据集特征张量形状: {dataset.patches_tensor.shape}")
    print(f"数据集特征张量类型: {dataset.patches_tensor.dtype}")
    print(f"数据集特征是否有NaN: {torch.isnan(dataset.patches_tensor).any()}")
    ...
    # 首先需要导入你的CNN模型定义
    try:
        from models.cnn import CNNEncoderFC  # 改成CNN模型
    except ImportError:
        print("错误：无法导入模型定义")
    ...
    with torch.no_grad():
        ...
        print(f"Patch数据形状: {patches_tensor.shape}")
        
        # 先测试一个小批量
        test_batch = patches_tensor[:10].to(device)
    ...
    ds.attrs["description"] = "CNN predicted ocean depth"
    ds.attrs["model"] = "ResidualCNNEncoder"
    ds.attrs["lon_range"] = str(LON_RANGE)
    ds.attrs["lat_range"] = str(LAT_RANGE)
    
    nc_path = OUTPUT_DIR / "predicted_bathymetry_cnn.nc"
    ...
    vmin, vmax = np.percentile(depth_grid[~np.isnan(depth_grid)], [5, 95])
    fig_save = plt.figure(figsize=(12, 8))
    im = plt.pcolormesh(dataset.lon, dataset.lat, masked_data, shading='auto', cmap='viridis_r', vmin=vmin, vmax=vmax)
    plt.colorbar(im, label='Depth (m)')
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    plt.title(f'CNN Predicted Bathymetry\nRange: {vmin:.0f} to {vmax:.0f} m')
    plt.tight_layout()
    png_path = OUTPUT_DIR / "predicted_bathymetry_cnn.png"
    ...
    if positive_count > 0:
        ...
        # 创建大于0区域的标记图
        print("\n生成大于0区域标记图...")
        fig_positive = plt.figure(figsize=(12, 8))
        
        # 基础图像
        im_positive = plt.pcolormesh(dataset.lon, dataset.lat, masked_data, 
                                    shading='auto', cmap='viridis_r',
                                    vmin=vmin, vmax=vmax)
        plt.colorbar(im_positive, label='Depth (m)')
        ...
        plt.xlabel('Longitude')
        plt.ylabel('Latitude')
        plt.title(f'CNN Predicted Bathymetry with Depth>0 Highlighted (Red)\nTotal >0: {positive_count} points')
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        
        positive_path = OUTPUT_DIR / "positive_depth_points_cnn.png"
        plt.savefig(positive_path, dpi=300, bbox_inches='tight')
        print(f"大于0区域标记图已保存: {positive_path}")
        ...
        plt.suptitle(f'CNN Analysis of Positive Depth Predictions (Total >0: {positive_count})', fontsize=14)
        plt.tight_layout()
        
        # 保存分析图
        analysis_path = OUTPUT_DIR / "positive_depth_analysis_cnn.png"
    ...
    plt.pcolormesh(dataset.lon, dataset.lat, masked_data, shading='auto', cmap='viridis_r', vmin=vmin, vmax=vmax)
    plt.colorbar(label='Depth (m)')
    plt.xlabel('Longitude')
    plt.ylabel('Latitude')
    plt.title(f'CNN Predicted Bathymetry - Final Result\nRange: {vmin:.0f} to {vmax:.0f} m')
    plt.tight_layout()
    plt.show()

if __name__ == "__main__":
    MODEL_PATH = "./checkpoints/cnn/best_model.pt"  # 修改为CNN模型路径
